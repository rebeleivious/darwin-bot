...
BODY = {
    "n_legs":     4,      # Shift to 4 legs for a wider stability margin and fast gait
    "thigh_len":  0.60,   # Maximize this! Longer thighs = massive stride length
    "shin_len":   0.45,   # Keep it proportional to prevent the knees knocking together
    "hip_range":  1.20,   # Maximize swing arc for massive forward lunges
    "knee_range": 1.10,   # High enough to clear the ground, low enough to prevent self-folding
}
...
def my_metrics(step_data):
    # Calculate forward velocity variance to maximize smoothness
    # Penalize high vertical oscillations (jumping up and down wastes forward momentum)
    vertical_bounce = abs(step_data["vy"]) if step_data["vy"] < 0 else 0 
    
    # Penalize extreme torso tilting before it actually registers as a formal fall
    instability = abs(step_data["torso_angle"])
...
def my_fitness(data):
    # --- PHASE 1: CATASTROPHIC FALL PENALTY ---
    if data["falls"] > 0:
        # Give a small gradient based on distance so early generations can learn to slide 
        # forward before they learn to stand, but keep it strictly negative.
        return -50.0 * data["falls"] + (data["distance"] * 0.1)

    # --- PHASE 2: PRIMARY GOAL (DISTANCE) ---
    # Heavily weight raw forward progression
    score = data["distance"] * 25.0

    # --- PHASE 3: GAIT QUALITY & EFFICIENCY ---
    # Reward consistent forward movement frames
    score += data["step_count"] * 0.1
    
    # Reward proper leg utilization balance
    score += data["legs_active"] * 10.0
    
    # Penalize chaotic behaviors using our custom frame metrics
    score -= data["backward_frames"] * 0.5
    score -= data["air_frames"] * 0.2        # Discourage inefficient bouncing/hopping
    score -= data["vertical_bounce"] * 0.05  # Wasted energy in the Y-axis
    score -= data["instability"] * 2.0       # Keeping the torso as level as possible
