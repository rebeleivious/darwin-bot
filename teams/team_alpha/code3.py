...
BODY = {
    "n_legs":     4,      # Quadruped - Stable base for 100 generations
    "thigh_len":  0.55,   # Increased from 0.45 for a larger, more powerful stride
    "shin_len":   0.44,   # Increased from 0.38 for maximum ground reach
    "hip_range":  1.10,   # Increased from 0.85 for a wider leg swing arc
    "knee_range": 1.20,   # Increased from 1.00 for stronger push-offs
}
...
def my_fitness(data):
    ...
    # Keeps fallen agents strictly at the bottom of the genetic pool.
    if data["falls"] > 0:
        return -500.0 * data["falls"] + data["distance"] * 0.1

    # ── PART B: Reward High-Velocity Forward Progression ──────────────
    
    # 1. Primary Driver: Distance is amplified to dominate selection
    score = data["distance"] * 1000.0  
    
    # 2. Velocity Driver: Frame-by-frame forward speed gradient
    score += data["fwd_speed"] * 0.1  
    
    # 3. Momentum Reward: Reward taking active steps (Max 600 * 0.5 = 300)
    score += data["step_count"] * 0.5  
    
    # 4. Structural Support: Reward using legs properly to prevent dragging
    score += data["legs_active"] * 200.0  

    # ── PART C: Additive Style Penalties & Bonuses ────────────────────
    
    # Additive Smoothness: Encourages efficiency without throttling top speed
    score += data["smoothness"] * 300.0  
    
    # Air Frames: Penalize bouncing or hopping off the ground
    score -= data["air_frames"] * 2.0  
    
    # Backward Frames: Severely penalize moving in reverse
    score -= data["backward_frames"] * 10.0  
    
    # Torso Instability: Penalize excessive wobbling or extreme tilting
    score -= data["torso_instability"] * 20.0  
...
def my_metrics(step_data):
    ...
    return {
        # Track backward movement frames
        "backward_frames": 1 if step_data["vx"] < -2 else 0,
        
        # Capture raw forward velocity
        "fwd_speed": step_data["vx"] if step_data["vx"] > 0 else 0,

        # Track airborne phase to prevent chaotic hopping
        "air_frames": 1 if step_data["feet_down"] == 0 else 0,
        
        # Torso Instability: Allows a natural lean up to ~0.20 rad (~11 degrees).
        # Anything beyond that gets tracked linearly to penalize heavy instability.
        "torso_instability": abs_tilt if abs_tilt > 0.20 else 0.0,
    }
