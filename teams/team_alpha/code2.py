...
# ══════════════════════════════════════════════════════════════════════
#  STEP 1 — DESIGN YOUR BODY (THE CHAMPION QUADRUPED)
# ══════════════════════════════════════════════════════════════════════
BODY = {
    "n_legs":     4,      # Your winning 4-legged foundation
    "thigh_len":  0.60,   # MAX upper leg length for massive stride leverage
    "shin_len":   0.50,   # MAX lower leg length for ultimate ground reach
    "hip_range":  1.20,   # MAX hip swing for wide, aggressive strides
    "knee_range": 1.10,   # Powerful knee extension to propel the mass forward
}
...
def my_fitness(data):
    # ── RULE 1: THE FAULT ELIMINATION LINE ───────────────────────────
    # If the quadruped flips completely over or collapses, it gets culled.
    # We keep a tiny gradient so early random twitches can evolve upward.
    if data["falls"] > 0:
        return -2000.0 * data["falls"] + (data["distance"] * 0.05)

    # ── RULE 2: EXPONENTIAL VELOCITY ENGINE ──────────────────────────
    dist = data["distance"]
    
    # Base distance reward
    distance_reward = dist * 60.0
    
    # Hyper-charge elite outliers exponentially. The further it goes, the more explosive the reward.
    if dist > 5.0:
        distance_reward += (dist - 5.0) ** 2 * 45.0
    if dist > 15.0:
        distance_reward += (dist - 15.0) ** 3 * 10.0  # Destroys any local minima ceilings

    # 2. Frame-by-Frame Velocity Consistency
    # Reward the total accumulated forward velocity to discourage stutter-stepping
    velocity_reward = data.get("forward_velocity_sum", 0) * 0.3
    smoothness_bonus = data["smoothness"] * 50.0

    # 3. Active Leg Drive (Anti-Gliding / Anti-Sliding)
    # Force the creature to actively step. A creature sliding on its stomach 
    # gets high distance but near-zero step counts and active legs.
    gait_reward = (data["step_count"] * 0.2) + (data["legs_active"] * 75.0)

    # 4. Behavioral Deficit Penalties
    backward_penalty  = data.get("backward_frames", 0) * 1.0
    airborne_penalty  = data.get("air_frames", 0) * 0.2        # Discourages useless bouncing
    belly_drag_penalty = data.get("low_torso_frames", 0) * 0.8  # STOPS it from sliding on its chest

    # Assembly
    total_rewards   = distance_reward + velocity_reward + smoothness_bonus + gait_reward
    total_penalties = backward_penalty + airborne_penalty + belly_drag_penalty

    fitness_score = total_rewards - total_penalties

    # Give stable, upright runners an unassailable baseline floor
    return max(100.0, fitness_score)
...
def my_metrics(step_data):
    return {
        # Tracks raw horizontal velocity directly per frame
        "forward_velocity_sum": step_data["vx"] if step_data["vx"] > 0 else 0.0,
        
        # Track frames where the creature drifts backwards
        "backward_frames": 1 if step_data["vx"] < -3 else 0,

        # Track frames where all 4 feet leave the ground completely
        "air_frames": 1 if step_data["feet_down"] == 0 else 0,

        # STOPS BELLY SLIDING: If the torso drops too close to the ground, penalize it frame-by-frame
        "low_torso_frames": 1 if step_data["torso_height"] < 95 else 0,
    }
